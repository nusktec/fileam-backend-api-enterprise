...
import { pitFilingService } from "../services/pitFilingService";
...
export const getPitCalculation = async (req: IRequest, res: Response) => {
  try {
    ensureValid(req);
    const userId = getAuthUserId(req);
    const year = Number(req.query.year);
    const data = await pitFilingService.getCalculation(userId, year);
    res.status(200).json({ data });
  } catch (error) {
    if (replyError(res, error)) return;
    res.status(500).json({ message: "Failed to get PIT calculation", error: "SERVER_ERROR" });
  }
};

export const submitPitFiling = async (req: IRequest, res: Response) => {
  try {
    ensureValid(req);
    const userId = getAuthUserId(req);
