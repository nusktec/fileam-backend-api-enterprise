...
export async function consultantOnboardingStep1(
  req: IRequest,
  res: Response,
): Promise<void> {
  ...
  if (
    ...
    res
      ...
      .json(
        outJson(
          false,
          "businessStructure, firmName, registrationType, countryOfRegistration are required",
          null,
        ),
      );
    return;
  ...
  const data: Step1Body = {
    ...
    rcNumber:
      body.rcNumber != null
        ? String(body.rcNumber).trim() || undefined
        : undefined,
    yearOfIncorporation:
      body.yearOfIncorporation != null &&
      hasNumber(Number(body.yearOfIncorporation))
...
export async function consultantOnboardingStep2(
  req: IRequest,
  res: Response,
): Promise<void> {
  ...
  const numberOfPartners =
    body.numberOfPartners != null ? Number(body.numberOfPartners) : 0;
  const principal = body.principalPartner as
    | Record<string, unknown>
    | undefined;
  if (
    !principal ||
    !hasString(principal.fullName) ||
    !hasString(principal.email) ||
    !hasString(principal.phone)
  ) {
    res
      .status(HttpStatusCode.BAD_REQUEST)
      .json(
        outJson(
          false,
          "principalPartner.fullName, .email, .phone are required",
          null,
        ),
      );
    return;
  ...
  const certifications = Array.isArray(principal.certifications)
    ? (principal.certifications as Array<Record<string, unknown>>).map((c) => ({
        qualificationName: hasString(c.qualificationName)
          ? c.qualificationName
          : String(c.qualificationName ?? ""),
        issuingBody: hasString(c.issuingBody)
          ? c.issuingBody
          : String(c.issuingBody ?? ""),
        year:
          c.year != null && hasNumber(Number(c.year))
            ? Number(c.year)
            : undefined,
  ...
  const data: Step2Body = {
    ...
    principalPartner: {
      ...
      yearsOfExperience:
        principal.yearsOfExperience != null &&
        hasNumber(Number(principal.yearsOfExperience))
  ...
  try {
    ...
    res
      ...
      .json(
        outJson(true, "Step 2 saved", { currentStep: 2, session: updated }),
      );
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
...
export async function consultantOnboardingStep3(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  ...
  const additionalPartners = Array.isArray(body.additionalPartners)
    ? (body.additionalPartners as Array<Record<string, unknown>>).map((ap) => ({
        partnerName: hasString(ap.partnerName)
          ? (ap.partnerName as string).trim()
          : String(ap.partnerName ?? ""),
        role: hasString(ap.role)
          ? (ap.role as string).trim()
          : String(ap.role ?? ""),
        yearsOfExperience:
          ap.yearsOfExperience != null &&
          hasNumber(Number(ap.yearsOfExperience))
        ...
        certifications: Array.isArray(ap.certifications)
          ? (ap.certifications as Array<Record<string, unknown>>).map((c) => ({
              qualification: hasString(c.qualification)
                ? c.qualification
                : String(c.qualification ?? ""),
              issuingBody: hasString(c.issuingBody)
                ? c.issuingBody
                : String(c.issuingBody ?? ""),
              national: c.national != null ? String(c.national) : undefined,
  ...
  try {
    ...
    res
      .status(HttpStatusCode.OK)
      .json(
        outJson(true, "Step 3 saved", { currentStep: 3, session: updated }),
      );
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
...
export async function consultantOnboardingStep4(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  const body = req.body as Record<string, unknown>;
  if (!hasString(body.primaryState) || !hasString(body.businessSizeServed)) {
    res
      .status(HttpStatusCode.BAD_REQUEST)
      .json(
        outJson(
          false,
          "primaryState and businessSizeServed are required",
          null,
        ),
      );
    return;
  ...
  try {
    ...
    res
      .status(HttpStatusCode.OK)
      .json(
        outJson(true, "Step 4 saved", { currentStep: 4, session: updated }),
      );
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
...
export async function consultantOnboardingStep5(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  ...
  if (!hasString(body.billingOption)) {
    res
      .status(HttpStatusCode.BAD_REQUEST)
      .json(outJson(false, "billingOption is required", null));
    return;
  }
  const perFilingReminderConfig = Array.isArray(body.perFilingReminderConfig)
    ? (body.perFilingReminderConfig as Array<Record<string, unknown>>).map(
        (r) => ({
          filing: String(r.filing ?? ""),
          frequency: String(r.frequency ?? ""),
          reminderDates: Array.isArray(r.reminderDates)
            ? (r.reminderDates as number[])
            : [],
        }),
      )
    : undefined;
  const data: Step5Body = {
    ...
    enableAutomatedComplianceReminders: Boolean(
      body.enableAutomatedComplianceReminders,
    ),
  ...
  try {
    ...
    res
      .status(HttpStatusCode.OK)
      .json(
        outJson(true, "Step 5 saved", { currentStep: 5, session: updated }),
      );
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
...
export async function consultantOnboardingStep6(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  ...
  if (!hasString(body.paymentMethod)) {
    res
      .status(HttpStatusCode.BAD_REQUEST)
      .json(outJson(false, "paymentMethod is required", null));
    return;
  }
  const data: Step6Body = {
    ...
    bankAccountNumber:
      body.bankAccountNumber != null
        ? String(body.bankAccountNumber).trim() || undefined
        : undefined,
    warrantApproval:
      body.warrantApproval != null
        ? String(body.warrantApproval).trim() || undefined
        : undefined,
    selfRemittance:
      body.selfRemittance != null
        ? String(body.selfRemittance).trim() || undefined
        : undefined,
  };
  try {
    ...
    res
      .status(HttpStatusCode.OK)
      .json(
        outJson(true, "Step 6 saved", { currentStep: 6, session: updated }),
      );
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
...
export async function consultantOnboardingStep7(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  ...
  const data: Step7Body = {
    cacDocumentUrl:
      body.cacDocumentUrl != null
        ? String(body.cacDocumentUrl).trim() || undefined
        : undefined,
    principalPartnerIdUrl:
      body.principalPartnerIdUrl != null
        ? String(body.principalPartnerIdUrl).trim() || undefined
        : undefined,
    professionalCertificateUrl:
      body.professionalCertificateUrl != null
        ...
        : undefined,
    amlDocumentUrl:
      body.amlDocumentUrl != null
        ? String(body.amlDocumentUrl).trim() || undefined
        : undefined,
    firmProfileUrl:
      body.firmProfileUrl != null
        ? String(body.firmProfileUrl).trim() || undefined
        : undefined,
...
export async function consultantOnboardingProfile(
  req: IRequest,
  res: Response,
): Promise<void> {
  const session = req.consultantOnboardingSession!;
  try {
    ...
    if (!full) {
      res
        ...
        .json(outJson(false, "Session not found", null));
      return;
    }
    res
      .status(HttpStatusCode.OK)
      .json(outJson(true, "Consultant onboarding profile", full));
  } catch (e) {
    res
      .status(HttpStatusCode.INTERNAL_SERVER_ERROR)
